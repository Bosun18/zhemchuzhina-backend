...
### Уведомления
- **Email**: при регистрации гость получает приветственное письмо, а администраторы — письмо о новом пользователе (адреса настраиваются в админке); при создании бронирования/отзыва администраторам приходит письмо о необходимости обработки; при подтверждении брони, отмене брони, одобрении и отклонении отзыва гость получает письмо на почту из профиля, а директор дополнительно уведомляется при отмене брони и отклонении отзыва; адреса для административных уведомлений настраиваются на странице «Настройки уведомлений» в админ-панели
- **Колокольчик (database notifications)**: те же события дублируются всплывающими уведомлениями со счётчиком непрочитанных — отображаются в Filament-панели (раздел уведомлений, опрос раз в 30 секунд) и доступны фронтенду через REST API
